...
const Layout = ({ children }) => {
  ...
  const isAdmin = roleOf(utente) === "ADMIN";

  // Forza admin sempre su /admin/utenti (oltre ai Route Guard)
  useEffect(() => {
    if (isAdmin && location.pathname !== "/admin/utenti") {
      navigate("/admin/utenti", { replace: true });
    }
          ...
          {isAdmin ? (
            ...
            <>
              <li className="nav-item">
                <NavLink to="/admin/utenti" className="nav-link text-white d-flex align-items-center">
                  <i className="bi bi-people me-2"></i>
                  {isSidebarOpen && 'Gestione utenti'}
                </NavLink>
              </li>
            </>
          ) : (
            // --- MENU STANDARD (NON ADMIN) ---
            <>
              <li className="nav-item">
